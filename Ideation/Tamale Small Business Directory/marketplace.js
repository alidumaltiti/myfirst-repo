...
function filterProvidersByCategory(category) {
    ...
    renderProviderGrid(filteredProviders.slice(0, providersPerPage));
...
function initializeProviderGrid() {
    renderProviderGrid(providersData.slice(0, providersPerPage));
}

function renderProviderGrid(providers, clearGrid = true) {
...
function createProviderCard(provider) {
    ...
    // Availability badge
    const availabilityClass = provider.availability === 'available' ? 'available' : (provider.availability === 'busy' ? 'busy' : 'offline');

    card.innerHTML = `
        <div class="relative">
            <img src="${provider.photo}" alt="${provider.name}" class="w-full h-40 object-cover rounded-lg mb-4">
            <span class="availability-badge ${availabilityClass}" title="${provider.availability}"></span>
        </div>
        <h3 class="font-display text-xl font-bold mb-1">${provider.name}</h3>
        <div class="flex items-center gap-3 mb-3">
            <div class="rating-stars">${'⭐'.repeat(Math.round(provider.rating))}</div>
            <div class="text-sm text-gray-500">(${provider.reviews} reviews)</div>
        </div>
        <p class="text-gray-700 mb-3">${provider.description}</p>
        <div class="flex items-center justify-between mt-4">
            <div class="text-sm price-tag">${provider.price}</div>
            <div class="flex gap-2">
                <button onclick="viewProfile(${provider.id})" class="btn-secondary px-3 py-2 rounded-md">View</button>
                <button onclick="requestQuote(${provider.id})" class="btn-primary px-3 py-2 rounded-md">Quote</button>
            </div>
        </div>
    `;
...
function loadMoreProviders() {
    ...
    const nextProviders = providersData.slice(start, end);

    if (nextProviders.length > 0) {
        renderProviderGrid(nextProviders, false);
    } else {
        showNotification('All available professionals loaded!', 'success');
        document.querySelector('button[onclick="loadMoreProviders()"]').style.display = 'none';
    }
}
...
function initializeNavigation() {
    ...
    // Login button (nav) - open login modal
